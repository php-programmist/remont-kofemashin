...
class ImporterController extends AbstractController
{
    ...
    /**
     * @Route("/brands-images", name="brands_images")
     */
    public function brands_images(BrandRepository $brand_repository)
    {
        
        $em          = $this->getDoctrine()->getManager();
        $project_dir = $this->getParameter('kernel.project_dir');
        
        $img_folder = $project_dir . '/public_html/img/brand_image/';
        $brands     = $brand_repository->findAll();
        
        foreach ($brands as $brand) {
            $file_name = $brand->getAlias() . '.jpg';
            if (file_exists($img_folder . $file_name)) {
                $brand->setImage($file_name);
            } else {
                $file_name = $brand->getAlias() . '.png';
                if (file_exists($img_folder . $file_name)) {
                    $brand->setImage($file_name);
                }else{
                    $file_name = $brand->getAlias() . '.jpeg';
                    if (file_exists($img_folder . $file_name)) {
                        $brand->setImage($file_name);
                    }
                }
